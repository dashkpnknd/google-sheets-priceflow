...
const RMG = {
  ...
  sheets: ['телефоны', 'аксессуары', 'макбуки', 'аймаки', 'айпады', 'часы', 'наушники', 'пс', 'дайсон', 'камеры', 'колонки'],
...
function rmgWrite_(sheet, products) {
  rmgRemoveCountryColumns_(sheet); const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0], layouts = rmgLayouts_(headers); if (!layouts.length) throw new Error('На листе «' + sheet.getName() + '» не найдена колонка Price/Цена.');
