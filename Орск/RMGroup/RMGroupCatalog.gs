...
function rmgInfo_(name) { const text = String(name || ''), model = /\b(iPhone\s+(?:Air|\d+(?:e)?(?:\s+(?:Pro Max|Pro|Plus|mini|Air))?)|iPad\s+[^\d]*\d+|MacBook\s+(?:(?:Air|Pro|Neo)\s+)?\d+|Apple Watch\s+[^\d]*\d+mm|Galaxy\s+[A-Z]\d+(?:\+|\s+(?:Ultra|FE|Plus))?|Honor\s+[\w-]+(?:\s+(?:Pro|Lite|X\d+))?)/i.exec(text), specs = /(\d{1,2})\s*\/\s*(\d{1,4})(?:\s*(гб|gb|тб|tb))?/i.exec(text), memory = specs ? null : /(?:\b(64|128|256|512|1024|2048)\s*(гб|gb|тб|tb)?\b|\b(1|2)\s*(гб|gb|тб|tb)\b)/i.exec(text), unit = function(n, u) { return n + ' ' + String(u || 'GB').toUpperCase().replace('GB', 'ГБ').replace('TB', 'ТБ'); }; return { model: model ? model[1] : '', ram: specs ? unit(specs[1], 'GB') : '', memory: specs ? unit(specs[2], specs[3]) : memory ? unit(memory[1] || memory[3], memory[2] || memory[4]) : '', sim: /sim\s*\+\s*e\s*-?sim/i.test(text) ? 'SIM + eSIM' : /e\s*-?sim/i.test(text) ? 'eSIM' : '', color: rmgColor_(text) }; }
function rmgColor_(value) { const v = rmgNorm_(value), pairs = [['rose gold','розовое золото'],['space gray','серый космос'],['natural','натуральный'],['desert','пустынный'],['ultramarine','ультрамарин'],['teal','бирюзовый'],['midnight','полуночный'],['starlight','сияющая звезда'],['lavender','лавандовый'],['sage','шалфейный'],['black','черный'],['white','белый'],['blue','синий'],['pink','розовый'],['green','зеленый'],['silver','серебристый'],['yellow','желтый'],['gray','серый'],['purple','фиолетовый'],['orange','оранжевый'],['red','красный'],['gold','золотой'],['черный','черный'],['белый','белый'],['синий','синий'],['розовый','розовый'],['красный','красный']]; const hit = pairs.find(function(pair) { return new RegExp('\\b' + pair[0] + '\\b', 'i').test(v); }); return hit ? rmgAvitoColor_(v, hit[1]) : ''; }
...
function rmgWrite_(sheet, products) {
  rmgRemoveCountryColumns_(sheet); const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0]; rmgValidateSchema_(sheet, headers); const layouts = rmgLayouts_(headers); if (!layouts.length) throw new Error('На листе «' + sheet.getName() + '» не найдена колонка Price/Цена.');
