...
function rmgName_(item) {
  const category = String(item.category || '').trim(), name = String(item.name || '').trim();
...
function rmgCategory_(value) { const v = rmgNorm_(value); if (/airpods|earpods|buds|наушник|headphone/.test(v)) return 'наушники'; if (/аксессуар|чехол|case\b|кабель|заряд|адаптер|держател|ремеш|gamepad|controller|док\b|подставк|дисковод/.test(v)) return 'аксессуары'; if (/macbook/.test(v)) return 'макбуки'; if (/\bimac\b/.test(v)) return 'аймаки'; if (/ipad|galaxy tab|tablet/.test(v)) return 'айпады'; if (/apple watch|galaxy watch|\bwatch\b/.test(v)) return 'часы'; if (/dyson/.test(v)) return 'дайсон'; if (/playstation|\bps[345]\b|xbox/.test(v)) return 'пс'; if (/iphone|samsung|galaxy|pixel|xiaomi|redmi|honor|huawei|oneplus|realme|oppo|vivo/.test(v)) return 'телефоны'; return 'прочее'; }
