/**
 * Краснодар / ru:Store — первый этап каталога и постановка второго этапа.
 * Авторизованный сервер читает весь доступный Telegram-канал, а этот скрипт
 * получает только свежий read-only снимок. Первый этап заполняет готовый
 * каталог; второй — отдельным запуском обновляет только Price нового шаблона.
 */
const TC = {
  ...
  // Его структура остаётся неизменной; matcher пишет только Price и причину.
  priceTemplate: { spreadsheetId: '1_QdZ0Z4zDKT7vmP-usVXY-yeZ6o6pUZXIALwlU6nSCI', headerRow: 1, firstDataRow: 2, sheets: {
  ...
  // Reuse the existing property from the working Краснодar read-only source;
  // its secret is never copied into code, logs, or the sidebar.
  props: { connected: 'KRS_CONNECTED', snapshotSecret: 'RUS_SNAPSHOT_SECRET', last: 'KRS_LAST', status: 'KRS_STATUS', templateLastReport: 'KRS_TEMPLATE_LAST_REPORT', supplierModels: 'KRS_READY_SUPPLIER_MODELS', sourceReport: 'KRS_SOURCE_REPORT' }
};
...
function getTelegramCatalogSetup() {
  ...
  return {
    project: 'ru:Store | Краснодар', channel: TC.channelTitle,
    connected: Boolean(p.getProperty(TC.props.connected)), lastSync: p.getProperty(TC.props.last) || '',
    status: p.getProperty(TC.props.status) || 'Не подключено'
  };
}

function saveTelegramCatalogSetup() {
  const p = PropertiesService.getScriptProperties();
  if (!p.getProperty(TC.props.snapshotSecret)) throw new Error('Не задано Script Property RUS_SNAPSHOT_SECRET.');
  p.setProperty(TC.props.connected, 'true');
  tcEnsureTrigger_();
...
function syncTelegramCatalog() {
  if (!PropertiesService.getScriptProperties().getProperty(TC.props.connected)) return { rows: 0, written: 0, skippedSheets: TC.sheets };
...
function syncTelegramCatalog_() {
  ...
  try {
    const p = PropertiesService.getScriptProperties();
    if (!p.getProperty(TC.props.connected)) throw new Error('Сначала сохраните подключение к Telegram-источнику.');
    const book = SpreadsheetApp.getActiveSpreadsheet();
    const sourceRows = tcFetchKrasnodarSnapshotRows_().filter(function(row) { return !tcIsAsis_(tcSourceText_(row)); });
    ...
    // Для Краснодара наценка и зеркальная строка 2 SIM не применяются.
    ...
    Object.keys(byCategory).forEach(function(category) {
      if (byCategory[category].length && TC.sheets.indexOf(category) < 0) throw new Error('Для категории «' + category + '» нет согласованной вкладки готового каталога. Каталог не изменён.');
    });
...
function tcRunKrasnodarRegressionTests() { return PriceFlowAvitoMatcher.runRegressionTests(); }
function tcAssertKrasnodarInvariants_() { return tcRunKrasnodarRegressionTests(); }
...
function tcIsAsis_(value) { return /(?:\b(?:asis|асис|cpo|цпо|open\s*box|refurb(?:ished)?|defect(?:ive)?|faulty|damaged|used|active)\b|(?:^|[^a-zа-яё])(?:б\/?у|бу)(?=$|[^a-zа-яё])|уцен|витрин|демо|(?:пред\s*)?актив(?:ир)?|распак|брак|вскрыт[а-яё]*\s*(?:короб|упаков)|поврежд[а-яё]*\s*(?:короб|упаков)|мят(?:ая|ый|ой|ую|ые|ых))/iu.test(String(value || '')); }
...
function tcFetchKrasnodarSnapshotRows_() {
  const props = PropertiesService.getScriptProperties();
  const secret = props.getProperty(TC.props.snapshotSecret);
  if (!secret) throw new Error('Не задано Script Property RUS_SNAPSHOT_SECRET.');
  const response = UrlFetchApp.fetch(TC.endpoint, {
    headers: { 'X-PriceFlow-Secret': secret }, muteHttpExceptions: true
  });
  if (response.getResponseCode() !== 200) {
    throw new Error('Сервис авторизованного Telegram-снимка вернул HTTP ' + response.getResponseCode() + '. Каталог не изменён.');
  }
  const payload = JSON.parse(response.getContentText());
  if (!payload || !Array.isArray(payload.posts)) throw new Error('Сервис вернул неверный формат снимка: ожидается posts[].');
  const rows = [], skipped = [], categories = {};
  payload.posts.forEach(function(post) {
    const parsed = krsParsePost_(post.text, String(post.id || ''));
    parsed.rows.forEach(function(row) { row.sourceRow = post.text; rows.push(row); });
    parsed.categories.forEach(function(category) { categories[category] = true; });
    parsed.skipped.forEach(function(item) { skipped.push('post ' + post.id + ': ' + item); });
  });
  if (!rows.length) throw new Error('В полном Telegram-снимке не найдено ни одной подтверждённой цены. Каталог не изменён.');
  props.setProperty(TC.props.sourceReport, JSON.stringify({
    at: new Date().toISOString(), refreshedAt: payload.refreshedAt || '', posts: payload.posts.length,
    rows: rows.length, categories: Object.keys(categories), skipped: skipped.slice(0, 200)
  }));
  return rows;
}
...
// Краснодарский канал доступен только авторизованному read-only серверу.
...
function krsParsePost_(text, postId) {
  ...
  lines.forEach(function(raw) {
    ...
    if (parsed) {
      (Array.isArray(parsed) ? parsed : [parsed]).forEach(function(row) { rows.push(row); seen(row.name); });
      return;
    }
    if (krsLooksLikeProduct_(line)) { context = krsExpand_(context, line); seen(context); }
    else if (/^(iphone|ipad|macbook|airpods|(?:apple\s+)?watch|samsung|galaxy|pixel|xiaomi|redmi|honor|huawei|oneplus|realme|oppo|vivo|dyson|ps\d|playstation|xbox)/i.test(line)) {
      context = line.replace(/[.·]{3,}/g, '').trim(); seen(context);
    }
  });
  return { rows: rows.filter(function(row) { return row.category !== 'прочее'; }), categories: Object.keys(categories), skipped: skipped };
}
function krsInlineLine_(line, context, postId) {
...
function krsRowsForCountries_(name, price, postId) {
  ...
  if (!flags.length) return krsRow_(name, price, postId);
  const base = String(name).replace(/[\u{1F1E6}-\u{1F1FF}]{2}/gu, '').replace(/\s+/g, ' ').trim();
  return flags.map(function(flag) { return krsRow_(base + ' ' + flag, price, postId); });
}
function krsRow_(name, price, post) {
  const clean = String(name).replace(/^[\[\]()\s]+/, '').replace(/\s+/g, ' ').trim();
  return { name: clean, price: Number(price), category: krsCategory_(clean), post: post };
}
function krsExpand_(context, item) {
  const value = String(item).trim(), ctx = String(context || '').replace(/[.·•]{3,}/g, '').trim();
...
function krsLooksLikeProduct_(line) { return /\d/.test(line) && !/^(?:\d+\s*шт|\d+\+|\d{1,2}:\d{2})/i.test(line); }
function krsCategory_(value) {
  const text = tcNorm_(value);
  if (/\bsmart\s*tag(?:\d|\b)|\bsmarttag(?:\d|\b)|чехол|стекло|кабель|заряд|адаптер|держател|ремеш|powerbank|пауэрбанк/.test(text)) return 'прочее';
  if (/airpods|galaxy\s*buds|\bbuds\b|наушник|headphones?/.test(text)) return 'наушники';
  if (/apple\s*watch|galaxy\s*(watch|fit|ring)|\bwatch\b/.test(text)) return 'часы';
  if (/ipad|galaxy\s*tab|\btablet\b/.test(text)) return 'айпады';
  if (/macbook/.test(text)) return 'макбуки';
  if (/\bimac\b/.test(text)) return 'аймаки';
  if (/dyson/.test(text)) return 'дайсон';
  if (/playstation|\bps[345]\b|xbox|dualsense|gamepad|controller|геймпад/.test(text)) return 'пс';
  if (/iphone|samsung|galaxy|pixel|xiaomi|redmi|poco|honor|huawei|oneplus|realme|oppo|vivo|nothing|tecno/.test(text)) return 'телефоны';
  return 'прочее';
}
