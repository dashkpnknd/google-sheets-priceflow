...
test('recognises colours in any part of a Telegram item without inventing a missing one', () => {
  ...
  assert.equal(api.tcColor_('iPhone 16 Pro (Desert Titanium), eSIM'), 'пустынный');
  assert.equal(api.tcColor_('Pixel 9 Pro 12/256GB, Obsidian 🇮🇳'), 'обсидиан');
...
test('fills Android RAM separately in the standard right-hand phone block', () => {
  ...
  assert.deepEqual([...row], ['Pixel 7', '', 'США 🇺🇸', '128 ГБ', 'лимонный', '8 ГБ', 23500]);
});
