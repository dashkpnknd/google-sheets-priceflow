...
const source = fs.readFileSync(new URL('./PriceFlowAvitoMatcher.gs', import.meta.url), 'utf8') + '\n' + fs.readFileSync(new URL('./PriceFlowNobelCatalog.gs', import.meta.url), 'utf8') + '\n' + fs.readFileSync(new URL('./PriceFlowTemplateMatcher.gs', import.meta.url), 'utf8') + '\n' + fs.readFileSync(new URL('./TelegramCatalog.gs', import.meta.url), 'utf8') + `
globalThis.API={TC,tcChannel_,tcCategory_,tcPhone_,tcColor_,tcColorGroup_,tcModel_,tcAndroidTechnicalModifiers_,tcIsAsis_,tcLayouts_,tcTargetRow_,tcParsePost_,tcLine_,tcExpand_,tcSummary_,tcProductSort_,tcAddTwoSimMirror_,tcChooseCheapestCountry_,tcSupplierColorKey_,tcParseMarkupCsv_,tcApplyUlyanovskMarkup_,tcMarkupAmount_,tcMarkupKey_,tcAndroidMarkupKey_,PriceFlowAvitoMatcher,PriceFlowTemplateMatcher,PriceFlowNobelCatalog};`;
...
test('Nobel deployment is explicitly enabled and keeps its fallback module separate', () => {
  assert.equal(api.TC.nobel.enabled, true);
  assert.equal(api.TC.nobel.priceType, 'BASE');
  assert.match(source, /nobel:TC\.nobel/);
  assert.match(source, /PriceFlowNobelCatalog\.load/);
});
