...
const TC = {
  ...
  // Enabled only in this Nobel deployment bundle.  The current bundle makes
  // no requests to the public competitor catalogue.
  nobel: { enabled: true, priceType: 'BASE', sourceLabel: 'Цена из сайта: НОБЕЛЬ.РФ' },
...
function tcSyncPriceTemplate_() {
  const report = PriceFlowTemplateMatcher.sync({ city:'ulyanovsk', sourceSpreadsheet:SpreadsheetApp.getActiveSpreadsheet(), templateSpreadsheetId:TC.priceTemplate.spreadsheetId, headerRow:TC.priceTemplate.headerRow, firstDataRow:TC.priceTemplate.firstDataRow, sheets:TC.priceTemplate.sheets, nobel:TC.nobel, allowIphoneAirAlias:true, supplierModels:tcReadySupplierModels_() });
...
function tcPriceTemplateSummary_(report) {
  ...
  const nobel = Object.keys(report.sheets || {}).reduce(function(total, name) { return total + Number((report.sheets[name] || {}).nobelMatched || 0); }, 0);
  return 'Шаблон цен: обновлено ' + Number(report.updated || 0) + ', из НОБЕЛЬ.РФ: ' + nobel + '. ' + (reasons.length ? 'Пропуски — ' + reasons.join(', ') + '.' : 'Пропусков нет.');
}
